...
ttp_escape_char_hex_body(0, Cnt, Str,StrT, Str,StrT) :-
	4 =< Cnt,
 	!.
...
ttp_reverse([], []) :- !.
ttp_reverse([H | T], OutLst) :-
	!,
	ttp_reverse_impl(T, H, [], OutLst).

ttp_reverse_impl([], OutH, OutT, [OutH | OutT]) :- !.
ttp_reverse_impl([H | T], OutH, OutTn, OutLst) :-
	!,
	ttp_reverse_impl(T, H, [OutH | OutTn], OutLst).
